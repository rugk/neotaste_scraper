...
def main() -> int:
    ...
    if not isinstance(data, dict):
        print(f"::warning::Top-level JSON is not an object in {filePath}")
        return 400

    for city, entries in data.items():
        if isinstance(entries, list) and len(entries) > 0:
            print(f"OK: found non-empty city '{city}' in {filePath}")
            return 0
    ...
    print(f"::warning::All city lists are empty in {filePath}")
    return 1
